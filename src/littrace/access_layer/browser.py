...
def check_browser_act(config: LitTraceConfig) -> BrowserActStatus:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            errors.append(
                result.stderr.strip() or result.stdout.strip() or "browser-act --version failed"
            )
    ...
    if not errors:
        try:
            ...
            if browser_result.returncode == 0:
                ...
                diagnostics.append(
                    ...
                    or "browser-act browser list failed"
                )
        except Exception as exc:
        ...
        try:
            ...
            if session_result.returncode == 0:
                ...
                diagnostics.append(
                    session_result.stderr.strip()
                    or session_result.stdout.strip()
                    or "browser-act session list failed"
                )
        except Exception as exc:
...
def prewarm_chrome_direct(
    config: LitTraceConfig, wait_seconds: float = 1.5
) -> ChromeDirectPrewarmResult:
    if config.browser.default_browser_type != "chrome-direct":
...
def run_browser_act(
    ...
    try:
        ...
        return BrowserActRunResult(
            ...
            stdout=stdout,
            stderr=stderr,
            recoverable_window_closed=is_recoverable_browser_window_closed(f"{stdout}\n{stderr}"),
...
def _candidate_browser_act_paths() -> list[Path]:
    ...
    if sys.platform.startswith("win"):
        ...
        if local_app:
            paths.append(
                Path(local_app) / "uv" / "tools" / "browser-act-cli" / "Scripts" / executable
            )
    return paths
