...
from littrace.access_layer.browser import run_browser_act
...
def archive_authorized_pdf_response(
    ...
    if candidate is None:
        return _archive_with_fallbacks(
            config,
            paper,
            session_name,
            pdf_url,
            target,
            timeout_seconds,
            reason="No authorized application/pdf network response was found.",
    ...
    if body is None:
        return _archive_with_fallbacks(
            ...
            target,
            timeout_seconds,
            reason="PDF response body was not available from browser-act.",
    ...
    if not data.startswith(b"%PDF"):
        return _archive_with_fallbacks(
            ...
            timeout_seconds,
            reason=(
                "Authorized response was application/pdf, but browser-act exposed a PDF viewer shell "
...
def _archive_with_fallbacks(
    ...
    *,
    ...
    click = _download_pdf_by_browser_click(
        config, paper, session_name, pdf_url, target, timeout_seconds=timeout_seconds
    )
    if click.archived:
        return click
    ...
    return AuthorizedPdfArchiveResult(
        ...
        warning=warning,
        error=(
            f"{reason} browser click fallback: {click.error}; "
            f"browser-context fetch fallback: {browser_fetch.error}; "
            f"cookie HTTP fallback: {cookie_fetch.error}"
        ),
    )
...
def _download_pdf_by_browser_click(
    ...
    if isinstance(payload, dict) and payload.get("clicked") is False:
        return AuthorizedPdfArchiveResult(
            ...
            method="browser_click_download",
            error=str(
                payload.get("error") or "No PDF download control was found on the browser page."
            ),
        )
...
def _download_pdf_with_browser_cookies(
    ...
    browser_state = _read_browser_cookie_state(config, session_name, pdf_url, timeout_seconds)
...
def _read_browser_cookie_state(
    ...
    if result.returncode != 0:
        return _BrowserCookieState(
            error=result.stderr or result.stdout or "browser cookie eval failed"
        )
    payload_text = _last_nonempty_line(result.stdout)
    try:
        import json
