...
from littrace.evaluation.harnesses import HarnessResult, check_storyline_claims
...
def render_structured_storyline_report(
    workspace: LiteratureWorkspace,
    config=None,
) -> str:
    """Public storyline output. Always route through the publication gate."""

    from littrace.config import LitTraceConfig
    from littrace.publication import render_publication_storyline

    markdown, _ = render_publication_storyline(workspace, config or LitTraceConfig())
    return markdown


def _render_structured_storyline_report(workspace: LiteratureWorkspace) -> str:
    """Internal renderer for evaluators and the publication boundary only."""
