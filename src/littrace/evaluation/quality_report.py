...
def build_quality_report(config: LitTraceConfig, workspace: LiteratureWorkspace) -> QualityReport:
    ...
    verified_count = sum(report.verified_candidate_count > 0 for report in full_text_reports)
    ...
    metrics = {
        ...
        "verified_full_text_candidate_rate": verified_count / active_count if active_count else 0.0,
        "oa_pdf_candidate_rate": oa_pdf_count / active_count if active_count else 0.0,
        "login_required_candidate_rate": login_required_count / active_count
        if active_count
        else 0.0,
        ...
        "supplementary_link_count": float(
            sum(len(items) for items in workspace.supplementary_links.values())
        ),
    }
    warnings = [
