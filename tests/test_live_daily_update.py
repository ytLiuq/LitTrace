"""Opt-in end-to-end check for the real Sentinel daily update path.

This is intentionally separate from the fast suite. It performs live discovery,
publisher probing, Chrome/CDP downloads, and persists the same artifacts as the
``littrace sentinel run`` command.

Run with ``LITTRACE_LIVE_DAILY_TESTS=1`` when network access and Chrome are
available.
"""
