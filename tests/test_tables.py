...
def test_extract_performance_cells_uses_regex_when_llm_is_unconfigured(monkeypatch):
    async def _missing_key(*args, **kwargs):
        from littrace.llm import LLMReply

        return LLMReply(text="", used_llm=False, error="missing_api_key")

    monkeypatch.setattr("littrace.tables.chat_completion", _missing_key)
    workspace = LiteratureWorkspace(
        parsed_papers={
            "p1": {
                "parsed": True,
                "sections": [
                    {
                        "name": "Results",
                        "text": "The array achieved sensitivity 2.3 kPa-1 and response time 45 ms.",
                        "evidence": {"paper_id": "p1", "page": 5, "parser": "docling"},
                    }
                ],
            }
        }
    ...
    assert {cell.metric for cell in workspace.performance_cells} >= {
        "sensitivity",
        "response time",
    }
    assert any("regex-only fallback" in warning for warning in harness.warnings)
